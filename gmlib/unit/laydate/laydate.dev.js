...
//全局配置，如果采用默认均不需要改动
var config =  {
    path: stageJsServer+'/gmlib/unit/laydate/', //laydate所在路径
...
Dates.view = function(elem, options){
    ...
    if(!Dates.box){
        ...
        +'</div>'
        + Dates.viewtb
        +'<p class="birth-data">生日设置后，将不支持修改</p>'
        +'<div class="laydate-btn">'
            +'<a class="cal-btn" id="laydate_ok">设置成我的生日</a>'
        +'</div>'
        +'<a class="laydate_close"></a>'
        
        +'<div class="laydate_bottom" style="display:none;">'
          +'<ul id="laydate_hms">'
...
Dates.creation = function(ymd, hide){
    ...
    if(!hide){
        ...
        Dates.close();
        typeof Dates.options.closed === 'function' && Dates.options.closed(getDates); 
    }
...
Dates.events = function(){
    ...
    Dates.each(S('#laydate_table td'), function(i, elem){
        Dates.on(elem, 'click', function(ev){
            if(!Dates.hasClass(this, as[1])){
                Dates.stopmp(ev);
                Dates.creation([this.getAttribute('y')|0, this.getAttribute('m')|0, this.getAttribute('d')|0],false);
            }
    ...
    Dates.on(as.ok, 'click', function(){
        if(Dates.valid){
            var aa = $("#laydate_table .laydate_click");
             Dates.creation([aa[0].getAttribute('y')|0, aa[0].getAttribute('m')|0, aa[0].getAttribute('d')|0],true);
        }
    });
